...
//Creating a new canvas with given width and height
canvas_t *create_canvas(int width, int height) {
    ...
    //Allocating memory for each row and initialize to zero
...
//Freeing all memory associated with a canvas
...
//Getting the value of a pixel at (x, y); returns 0.0f if out of bounds
float get_pixel(canvas_t *c, int x, int y) {
    if (!c||x<0||x>=c->width||y<0||y>=c->height) return 0.0f;
    return c->pixels[y][x];
}

//Setting the value of a pixel at (x, y); clamps value to [0,1]
...
//Setting pixel with bilinear interpolation at floating-point coordinates (x, y)
...
// Drawing an anti-aliased line from (x0, y0) to (x1, y1) with given thickness
void draw_line_f(canvas_t *c, float x0, float y0, float x1, float y1, float thickness) {
    ...
    for (int n = 0; n <= steps; ++n){
        ...
        float px = x0 + t*dx;
        float py = y0 + t*dy;
        //Drawing a filled circle at each step for thickness
...
//Saving the canvas to a file in PGM (P2) format
